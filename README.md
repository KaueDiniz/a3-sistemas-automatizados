# Sistema Automatizado de Portão de Garagem

**Projeto A3 — Sistemas Automatizados**
Curso: Engenharia de Software
Unidade Curricular: Sistemas Automatizados
Instituição: Universidade São Judas Tadeu – USJT
Semestre: 2026/1
Professor: Prof. Robson Calvetti

---

## Integrante

| Nome | RA |
|------|----|
| Kauê Melo Diniz | 823130975 |

---

## Descrição

Desenvolvimento e simulação de um sistema automatizado para controle de portão de garagem utilizando lógica de CLP (Controlador Lógico Programável) em ambiente virtual.

---

## Ferramenta Utilizada

- [PLC Fiddle](https://www.plcfiddle.com) — simulador online de lógica Ladder

---

## Arquivos do Repositório

| Arquivo | Descrição |
|---------|-----------|
| [Relatorio_A3_Portao_Garagem_ABNT.pdf](Relatorio_A3_Portao_Garagem_ABNT.pdf) | Relatório técnico completo (padrão ABNT) |
| [Apresentacao_A3_Portao_Garagem.pptx](Apresentacao_A3_Portao_Garagem.pptx) | Apresentação em slides |
| [README.md](README.md) | Este arquivo |

---

## Entregáveis

- [x] Repositório GitHub
- [x] Relatório técnico (PDF)
- [x] Apresentação em slides (PPTX)
- [x] Simulação no PLC Fiddle

---

## Simulação

A lógica Ladder completa pode ser testada ao vivo no PLC Fiddle:
🔗 **[Abrir simulação](https://www.plcfiddle.com/fiddles/4c5b9150-3a33-47d7-8011-9ba8750eeeb1)**

---

© 2026 — Kauê Melo Diniz | USJT
